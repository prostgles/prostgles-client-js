...
import { getSymbolComments } from "./getSymbolComments";
...
export const getSerializableType = (
  ...
  if (checker.isArrayType(myType)) {
    ...
    if (itemType && checker.isTupleType(itemType)) {
      const tupleTypes =
        (itemType as unknown as { resolvedTypeArguments: ts.Type[] }).resolvedTypeArguments.map(
          (d: ts.Type) => {
            return getSerializableType(d, checker, visited, nextUnresolvedParentAliases, opts)
              .resolvedType;
          },
        ) ?? [];
      return withAlias({
    ...
    const resolvedItemType: TS_Type =
      itemType ?
        getSerializableType(itemType, checker, visited, nextUnresolvedParentAliases, opts)
          .resolvedType
  ...
  if ((myType.flags & ts.TypeFlags.Object) !== 0) {
    ...
    if (objectType.objectFlags & ts.ObjectFlags.Reference) {
      ...
      if (target.symbol?.escapedName === "Promise") {
        ...
        const resolvedInnerType =
          _innerType &&
          getSerializableType(_innerType, checker, visited, nextUnresolvedParentAliases, opts)
            .resolvedType;
    ...
    myType.getProperties().forEach((symbol) => {
      ...
      const propertyComments = getSymbolComments(symbol, checker);
...
export const resolveType = (
  ...
  const { resolvedType, visited } = getSerializableType(myType, checker, undefined, [], opts);
  console.log(visited);
