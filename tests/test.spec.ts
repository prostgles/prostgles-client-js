...
import { useEffectAsync, useIsMounted, type OnReadyParams } from "prostgles-client";
...
test("exports work", () => {
  const exportedTypes = Array.from(new Set([useEffectAsync, useIsMounted].map((v) => typeof v)));
