 ...
 */
export declare type Sync<Model = POJO> = <T = Model | POJO>(basicFilter: Partial<T>, options: SyncOptions, onChange: (data: (SyncDataItem<T>)[], delta?: Partial<T>[]) => any, onError?: (error: any) => void) => Promise<MultiSyncHandles<T>>;
/**
 * Creates a local synchronized record
 */
export declare type SyncOne<Model = POJO> = <T = Model | POJO>(basicFilter: Partial<T>, options: SyncOneOptions, onChange: (data: (SyncDataItem<T>), delta?: Partial<T>) => any, onError?: (error: any) => void) => Promise<SingleSyncHandles<T>>;
