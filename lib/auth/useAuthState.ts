...
import { useEffect, useState } from "../hooks/reactImports";
...
export const useAuthState = ({ auth }: { auth: AuthHandler }) => {
  ...
  const [result, setResult] = useState<Extract<FormData, { type: typeof state }>["result"]>();
  ...
  const onAuthCall = async () => {
    ...
    if (!res.success) {
      ...
      if (res.code === "totp-token-missing") {
        ...
        const errorMessage = res.message ?? getProperty(ERR_CODE_MESSAGES, res.code);
